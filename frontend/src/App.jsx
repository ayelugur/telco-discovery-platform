...
export default function App() {
  ...
  useEffect(() => {
    if (analysis.isComplete) setActiveTab('graph')
  }, [analysis.isComplete])
    ...
    <div className="flex flex-col h-screen bg-surface-900 overflow-hidden">
      <header className="bg-surface-800 border-b border-slate-700 px-6 py-3 flex items-center gap-4 shrink-0">
        ...
        <nav className="flex items-center gap-1 ml-8 bg-surface-900 rounded-xl p-1">
          {TABS.map(tab => {
            const Icon = tab.icon
            const ready = hasResult(tab)
            const isActive = activeTab === tab.id
            return (
              <button key={tab.id} onClick={() => ready && setActiveTab(tab.id)} disabled={!ready}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium
                  transition-all duration-200 whitespace-nowrap
                  ${isActive ? 'bg-brand-600 text-white shadow-lg shadow-brand-600/20'
                    : ready ? 'text-slate-400 hover:text-slate-200 hover:bg-surface-700'
                    : 'text-slate-700 cursor-not-allowed'}`}>
                <Icon className="w-3.5 h-3.5" />
                {tab.label}
                {ready && tab.result && <span className="w-1.5 h-1.5 rounded-full bg-green-400 shrink-0" />}
              </button>
            )
          })}
        </nav>
        ...
        <div className="ml-auto flex items-center gap-3">
          {analysis.isRunning && (
            <div className="flex items-center gap-2 text-xs text-yellow-400 bg-yellow-400/10 border border-yellow-400/20 rounded-lg px-3 py-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse" />
              Agents Running
            </div>
          )}
          {analysis.isComplete && (
            <div className="flex items-center gap-2 text-xs text-green-400 bg-green-400/10 border border-green-400/20 rounded-lg px-3 py-1.5">
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar assets={analysis.assets} onUpload={analysis.uploadAsset}
          onRemove={analysis.removeAsset} onReset={analysis.reset} isRunning={analysis.isRunning} />

        <main className="flex-1 overflow-hidden p-5">
          {activeTab === 'console' && (
            <AgentStream AGENTS={analysis.AGENTS} AGENT_LABELS={analysis.AGENT_LABELS}
              agentStatus={analysis.agentStatus} agentLogs={analysis.agentLogs}
              isRunning={analysis.isRunning} isComplete={analysis.isComplete} onRun={analysis.runAnalysis} />
          )}
          {activeTab === 'graph' && (
            <div className="h-full">
              ...
              <div className="h-[calc(100%-3rem)]"><KnowledgeGraph data={analysis.results.discovery} /></div>
            </div>
          )}
          {activeTab === 'appmap' && (
            <div className="h-full">
              ...
              <div className="h-[calc(100%-3rem)] overflow-y-auto"><ApplicationMap data={analysis.results.discovery} /></div>
            </div>
          )}
          {activeTab === 'risk' && (
            <div className="h-full">
              ...
              <div className="h-[calc(100%-3rem)]"><RiskHeatmap data={analysis.results.risk} /></div>
            </div>
          )}
          {activeTab === 'roadmap' && (
            <div className="h-full">
              ...
              <div className="h-[calc(100%-3rem)]">
                <MigrationRoadmap data={analysis.results.roadmap} aiData={analysis.results.ai_opportunities} />
              </div>
            </div>
          )}
        </main>
